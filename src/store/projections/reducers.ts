import {MutationState, MutationAction, MutationActionType, Vec3} from 'store/projections/types';
...
const originalRotation = payloadSetter(MutationActionType.SET_ORIGINAL_ROTATION);
const rotationDelta = payloadSetter(MutationActionType.SET_ROTATION_DELTA)

function translation(translation: Vec3 = [0, 0, 0], action: MutationAction): Vec3 {
  if(action.type !== MutationActionType.TRANSLATE)
    return translation;
